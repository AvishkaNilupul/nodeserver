...
const ANALYST_BASE = [
  ...
  "- `propose_webbot_split` — EXECUTABLE: split a web-farm bot into halves. When",
  "  the operator asks to SPLIT, CALL this (don't just describe it); an 'Approve &",
  "  run' button appears in the chat. Confirm id/game/count via db_group first.",
  "- `propose_webbot_repin` — EXECUTABLE: re-pin the EXACT accounts in web-farm",
  "  bot(s) to a different game (keeps the accounts together). Use for 'put these",
  "  accounts back on game X' / 'make these bots farm Y'. CALL it; don't just",
  "  describe. Both become one-tap buttons — file the action, never only prose.",
