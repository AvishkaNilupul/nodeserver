...
const { getStreamsLive } = require("./twitchWatch");
...
async function runOnce() {
  ...
  try {
    ...
    for (const c of campaigns) {
      ...
      // No channels to check → we cannot (and will not) gate this campaign:
      ...
      let lastError = "";
      if (!tokens.length) {
        // No token to check with → cannot judge. Fail toward farming: mark
        // watchable but leave checkedAt so botWaker sees a fresh-ish row. (A
        // truly stale row from a Scout outage is handled by botWaker's own
        // staleness check.)
        liveNow = true;
      ...
      const patch = {
        ...
        // darkSince: stamp when we transition live→dark, keep it while dark,
        // clear it while live.
        darkSince: liveNow
          ? null
          : prev && prev.darkSince
